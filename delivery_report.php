...
$sql_2 = " SELECT waybill_no, consignor_name, consignee_name, description, source, weight, total_charged, way_date, status 
	          FROM waybill
		...
		if ($status != "All")
		{
			...
			if($status == "deliverd")
			{ 
			$sql_2.= " AND (delivered_date BETWEEN '$fromdate' AND '$todate')";
			}
			else
			{
			   $sql_2.= " AND (status = 'out for delivery' OR status = 'at destination') AND delivered_date = '0000-00-00' ";
			}
		...
		{
			$sql_2.= "AND (status = 'out for delivery' OR status = 'at destination' OR status = 'delivered') ";
		}
...
$pdf->SetTitle("Delivery report",false);
...
$pdf->SetY(30);
$pdf->Cell(270, 20,'DELIVERY REPORT',0 ,1 ,"C");
...
$pdf->Cell(80, 10,'BRANCH:'.$dest,0 ,0 ,"C");
